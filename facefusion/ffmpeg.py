...
FFMPEG_PATH = r"D:\2027\SouthSound\kids\FL\venya\lib\site-packages\imageio_ffmpeg\binaries\ffmpeg-win-x86_64-v7.1.exe"
...
def run_ffmpeg(args: List[str]) -> subprocess.Popen[bytes]:
    # prefer explicit binary if set, otherwise fallback to system ffmpeg
    ffmpeg_bin = FFMPEG_PATH if FFMPEG_PATH else shutil.which('ffmpeg')
    if not ffmpeg_bin:
        logger.error(
            'ffmpeg binary not found (FFMPEG_PATH and system ffmpeg missing).',
            __name__
        )
        raise RuntimeError('ffmpeg not found')

    commands = [ffmpeg_bin, '-hide_banner', '-loglevel', 'info']
    ...
    # Stream stderr in real time to logger (useful when UI shows debug logs)
    try:
        while True:
            ...
            if line:
                try:
                    ...
                    text = str(line)
                # always log at debug level so UI picks it up when debug enabled
                logger.debug('[ffmpeg] ' + text, __name__)

        # ensure all remaining output consumed
        ...
        # if something unexpected happens, attempt to collect output
        try:
            stdout, stderr = process.communicate(timeout=2)
        except Exception:
            stdout, stderr = b'', b''
        logger.error(f'Error while running ffmpeg: {e}', __name__)

    # if debug mode, also parse final stderr text
    if state_manager.get_item('log_level') == 'debug' and stderr:
        for line in stderr.decode(errors='ignore').splitlines():
            if line.strip():
                logger.debug('[ffmpeg-final] ' + line.strip(), __name__)
...
def open_ffmpeg(args: List[str]) -> subprocess.Popen[bytes]:
    ...
    commands = [ffmpeg_bin, '-hide_banner', '-loglevel', 'quiet']
    commands.extend(args)

    # return process with pipes so caller can interact (read/write)
...
def log_debug(process: subprocess.Popen[bytes]) -> None:
    # safer debug extractor (non-blocking attempt to collect stderr)
    try:
        # attempt to read any available stderr
        if process.stderr:
            partial = process.stderr.read().decode(errors='ignore')
            for l in partial.splitlines():
                if l.strip():
                    logger.debug('[ffmpeg-stream] ' + l.strip(), __name__)
    except Exception:
...
def merge_video(target_path : str, output_video_resolution : str, output_video_fps : Fps) -> bool:
	...
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%08d')
	encoder = state_manager.get_item('output_video_encoder') or DEFAULT_ENCODER
	preset = state_manager.get_item('output_video_preset') or DEFAULT_PRESET
	quality = state_manager.get_item('output_video_quality') or DEFAULT_QUALITY

	state_manager.set_item('output_video_encoder', encoder)
	state_manager.set_item('output_video_preset', preset)
	state_manager.set_item('output_video_quality', quality)
