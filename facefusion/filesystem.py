...
def is_media_path(path: str) -> bool:
    """
    Надёжная проверка медиа-файла: видео или изображение.
    Аудио НЕ считаем медиа для FaceFusion — оно ломает пайплайн.
    """
    try:
        if not path:
            return False
        ...
        # 1) Попытка определить через mimetypes
        typ, _ = mimetypes.guess_type(path)
        if typ:
            # FaceFusion работает только с image/* и video/*
            return typ.startswith("image") or typ.startswith("video")

        # 2) fallback по расширению
