...
def process_step(job_id : str, step_index : int, step_args : Args) -> bool:
	try:
		# existing flow (wrapped in try to catch all unexpected exceptions)
		...
		# гарантированно залогируем полную трассировку и аккуратно остановим процесс-менеджер
		try:
			logger.error("Unhandled exception in process_step (job_id={}, step_index={}):".format(job_id, step_index), __name__)
			logger.error(traceback.format_exc(), __name__)
		except Exception:
			# в крайнем случае — просто напечатать в stderr
			print("Unhandled exception in process_step:", file=sys.stderr)
			traceback.print_exc()

		# если процесс был запущен — убедимся, что он завершён корректно
		try:
			if process_manager.is_processing() or process_manager.is_pending():
				process_manager.end()
		except Exception:
