# Safe import for gradio_rangeslider: fallback to gradio.Slider if not available
try:
    from gradio_rangeslider import RangeSlider  # type: ignore
    HAS_RANGE_SLIDER = True
except Exception:
    RangeSlider = None
    HAS_RANGE_SLIDER = False

def _make_range_slider(*args, **kwargs):
    if HAS_RANGE_SLIDER and RangeSlider is not None:
        return RangeSlider(*args, **kwargs)
    import gradio as _gr
    return _gr.Slider(*args, **kwargs)
...
def render() -> None:
	...
	with gradio.Group():
		...
		with gradio.Row():
			...
			FACE_SELECTOR_AGE_RANGE_SLIDER = _make_range_slider(
