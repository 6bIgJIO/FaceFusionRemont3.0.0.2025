...
def check_sfw_image(img: Image.Image):
    return img  # просто возвращаем изображение, никаких проверок
...
def build_face_model(image: Image.Image, name: str, save_model: bool = True, det_size=(640, 640)):
    ...
    # Ограничиваем det_size для стабильной работы DML
    det_size_fixed = (min(det_size[0], 512), min(det_size[1], 512))
    face_model = analyze_faces(image, det_size_fixed)

    # Если не найдено лиц, пробуем с половиной размера
    if len(face_model) == 0:
        det_size_half = (det_size_fixed[0] // 2, det_size_fixed[1] // 2)
